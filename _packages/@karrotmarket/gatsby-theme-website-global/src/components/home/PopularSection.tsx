...
const PopularSection: React.FC<ParallaxSectionProps> = (props) => {
  ...
  if (!(content.primary && articles) || articles?.length === 0) return <React.Fragment />;
      ...
      <Container>
        ...
        <ItemList>
          {articles.slice(0, 6).map(({ articleId, price, region, image }) => (
            <Item key={articleId} id={articleId} image={image} price={price} region={region} />
          ))}
        </ItemList>
...
const Item: React.FC<ItemProps> = ({ image, price, region, id }) => (
  <ItemContainer href={`/articles/${id}`}>
    <ItemImage src={image} width="100%" height="100%" alt="hot-article" />
