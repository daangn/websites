...
const MockupSection: React.FC<MockupSectionProps> = ({ content }) => {
    ...
    const { text, background_color, text_highlight_color, phone_mockup_component, text_on_right } = content.primary;
    const PhoneMockupComponent = PhoneMockup[phone_mockup_component as PhoneMockupType];
        ...
        <Section css={{ background: background_color }} ref={ref}>
            <Container textPosition={text_on_right ? "right" : "left"}>
                <Flex colCenterY alignItems={{ "@i": "center", "@md": "flex-start" }}>
                    ...
                    <Grid
                        gridTemplateColumns={{ "@i": "max-content", "@md": "repeat(2, max-content)" }}
                        columnGap={{ "@i": 0, "@md": 30 }}
                        rowGap={{ "@i": 10, "@md": 0 }}
