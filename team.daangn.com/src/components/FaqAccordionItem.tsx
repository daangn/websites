...
const FaqAccordionItem: React.FC<FaqAccordionItemProps> = ({ id, entry, className, onClick, onFocus, open = false }) => {
    return (
        <Container className={className}>
            <Header as="h2" size={{ "@sm": "sm" }}>
                <Button id={entry.id} onClick={() => onClick?.(id)} onFocus={() => onFocus?.(id)}>
                    {entry.question}
